...
#        fig.savefig('/Users/koen/Documents/Courses/AE5810 Thesis Space/Meetings/0901/L' + str(self.lagrangePointNr) + '_' + self.orbitType + '_' + str(self.orbitId) + '_eigenvector_subplots.png')
    ...
    def plot_mass_validation(self):
        ...
        for i in range(self.numberOfOrbitsPerManifold):
            ...
            w_u_min_first_state = self.W_U_min.xs(0).head(1).values[0]
            w_u_min_first_time = self.W_U_min.xs(i).head(1).index[0]
            ...
            for row in self.W_S_plus.xs(i).tail(len(self.W_S_plus.xs(i)-1)).iterrows():
                ...
                w_s_plus_time_diff = abs(w_s_plus_first_time - row[0])
                w_s_plus_delta_j.append(abs(abs(mass_theoretical)-(abs(w_s_plus_first_iom-w_s_plus_iom)/w_s_plus_time_diff)))
                w_s_plus_first_iom = w_s_plus_iom
                w_s_plus_first_time = row[0]
            for row in self.W_S_min.xs(i).tail(len(self.W_S_min.xs(i)-1)).iterrows():
                w_s_min_t.append(abs(row[0]))
                w_s_min_state = row[1].values
                w_s_min_iom =w_s_min_state[6]
                w_s_min_time_diff = abs(w_s_min_first_time - row[0])
                w_s_min_delta_j.append(abs(abs(mass_theoretical)-abs(w_s_min_first_iom-w_s_min_iom)/w_s_min_time_diff))
                w_s_min_first_iom = w_s_min_iom
                w_s_min_first_time = row[0]
            for row in self.W_U_plus.xs(i).tail(len(self.W_U_plus.xs(i)-1)).iterrows():
                ...
                w_u_plus_time_diff = abs(w_u_plus_first_time - row[0])
                w_u_plus_delta_j.append(abs(abs(mass_theoretical)-abs(w_u_plus_first_iom-w_u_plus_iom)/w_u_plus_time_diff))
                w_u_plus_first_iom = w_u_plus_iom
                w_u_plus_first_time = row[0]
            for row in self.W_U_min.xs(i).tail(len(self.W_U_min.xs(i)-1)).iterrows():
                w_u_min_t.append(abs(row[0]))
                w_u_min_state = row[1].values
                w_u_min_time = row[1].index
                w_u_min_iom = w_u_min_state[6]
                w_u_min_time_diff = abs(w_u_min_first_time - row[0])
                w_u_min_delta_j.append(abs(abs(mass_theoretical)-abs(w_u_min_first_iom-w_u_min_iom)/w_u_min_time_diff))
                w_u_min_first_iom = w_u_min_iom
                w_u_min_first_time = row[0]
