...
class DisplayAugmentedValidation:
    ...
    def plot_manifolds(self): #ADAPT self.orbitType if statements argument to horizontal to get rid of xz and yz projections
        ...
        if self.thrustRestriction == 'left':
            ...
            plt.suptitle('$L_' + str(self.lagrangePointNr) + '$ '+ '$\{ \mathcal{W}^{S \pm}, \mathcal{W}^{U \pm} \}^{ \\alpha = ' + self.thrustRestriction + '}_{' + self.thrustMagnitudeForPlotTitle + '}$' + ' - Orthographic projection at C = ' + str(np.round(self.C, 3)), size=self.suptitleSize)
...
#        fig.savefig('/Users/koen/Documents/Courses/AE5810 Thesis Space/Meetings/0901/L' + str(self.lagrangePointNr) + '_' + self.orbitType + '_' + str(self.orbitId) + '_eigenvector_subplots.png')
    ...
    def plot_iom_validation(self):
        ...
        ax5.semilogy(w_s_min_dy[w_s_min_dy['dy'] < 1e-10], c=self.plottingColors['W_S_min'],
                     label='$\mathbf{X}^i_n \; \\forall \; i \in \mathcal{W}^{S-}$', linestyle='--')
        ax5.semilogy(w_s_min_dx[w_s_min_dx['dx'] < 1e-10], c=self.plottingColors['W_S_min'], linestyle='--')
        ax5.semilogy(w_s_min_dm[w_s_min_dm['dm'] < 1e-10], c=self.plottingColors['W_S_min'], linestyle='--')
        ...
        ax5.semilogy(w_u_min_dy[w_u_min_dy['dy'] < 1e-10], c=self.plottingColors['W_U_min'], linestyle='--')

        ax5.set_ylabel('$|x^i_{t_f} - (1-\mu)|, \; |y^i_{t_f}|, \; |1-m^i_{t_f}| $[-]')  # \; \\forall i =0, 1, \ldots m \in \mathcal{W}
        ...
        for i in range(self.numberOfOrbitsPerManifold):
            ...
            for row in self.W_S_min.xs(i).iterrows():
                ...
                w_s_min_iom = computeIntegralOfMotion(w_s_min_state[0], w_s_min_state[1], w_s_min_state[2],
                                                     w_s_min_state[3], w_s_min_state[4], w_s_min_state[5], w_s_min_state[6], self.thrustMagnitude, self.thrustRestriction)
            ...
            for row in self.W_U_plus.xs(i).iterrows():
                ...
                w_u_plus_iom = computeIntegralOfMotion(w_u_plus_state[0], w_u_plus_state[1], w_u_plus_state[2],
                                                      w_u_plus_state[3], w_u_plus_state[4], w_u_plus_state[5], w_u_plus_state[6], self.thrustMagnitude, self.thrustRestriction)
            ...
            for row in self.W_U_min.xs(i).iterrows():
                ...
                w_u_min_iom = computeIntegralOfMotion(w_u_min_state[0], w_u_min_state[1], w_u_min_state[2],
                                                     w_u_min_state[3], w_u_min_state[4], w_u_min_state[5], w_u_min_state[6], self.thrustMagnitude, self.thrustRestriction)
    ...
    def plot_stopping_validation(self):
        ...
        if self.lagrangePointNr == 1:
            ...
            ax2.plot(w_s_plus_dm[w_s_plus_dm['dm'] < 1], c=self.plottingColors['W_S_plus'],
                         label='$\mathbf{X}^i_n \; \\forall \; i \in \mathcal{W}^{S+}$')
            ax2.plot(w_s_min_dm[w_s_min_dm['dm'] < 1], c=self.plottingColors['W_S_min'],
                         label='$\mathbf{X}^i_n \; \\forall \; i \in \mathcal{W}^{S-}$', linestyle='--')
        ...
        if self.lagrangePointNr == 2:
            ...
            ax2.plot(w_s_plus_dm[w_s_plus_dm['dm'] < 1e0], c=self.plottingColors['W_S_plus'],
                         label='$\mathbf{X}^i_n \; \\forall \; i \in \mathcal{W}^{S+}$')
            ax2.plot(w_s_min_dm[w_s_min_dm['dm'] < 1e-0], c=self.plottingColors['W_S_min'],
                         label='$\mathbf{X}^i_n \; \\forall \; i \in \mathcal{W}^{S-}$', linestyle='--')
    ...
    def plot_thrust_validation(self):
        ...
        for i in range(self.numberOfOrbitsPerManifold):
            ...
            for row in self.W_S_min.xs(i).iterrows():
                w_s_min_t.append(abs(row[0]))
                w_s_min_state = row[1].values
                w_s_min_iom = computeThrustAngle(w_s_min_state[3], w_s_min_state[4], w_s_min_state[5], w_s_min_state[6], self.thrustMagnitude, self.thrustRestriction)
                w_s_min_delta_j.append(w_s_min_iom - w_s_min_first_iom)
            for row in self.W_U_plus.xs(i).iterrows():
                w_u_plus_t.append(abs(row[0]))
                w_u_plus_state = row[1].values
                w_u_plus_iom = computeThrustAngle(w_u_plus_state[3], w_u_plus_state[4], w_u_plus_state[5], w_u_plus_state[6], self.thrustMagnitude, self.thrustRestriction)
                w_u_plus_delta_j.append(w_u_plus_iom - w_u_plus_first_iom)
            for row in self.W_U_min.xs(i).iterrows():
                w_u_min_t.append(abs(row[0]))
                w_u_min_state = row[1].values
                w_u_min_iom = computeThrustAngle(w_u_min_state[3], w_u_min_state[4], w_u_min_state[5], w_u_min_state[6], self.thrustMagnitude, self.thrustRestriction)
                w_u_min_delta_j.append(w_u_min_iom - w_u_min_first_iom)
    ...
    def plot_mass_validation(self):
        ...
        for i in range(self.numberOfOrbitsPerManifold):
            ...
            w_u_min_delta_j = []
            w_s_plus_first_state = self.W_S_plus.xs(0).head(1).values[0]
            w_s_plus_first_time = float(self.W_S_plus.xs(i).head(1).index[0])
            w_s_min_first_state = self.W_S_min.xs(0).head(1).values[0]
            w_s_min_first_time = float(self.W_S_min.xs(i).head(1).index[0])
            w_u_plus_first_state = self.W_U_plus.xs(0).head(1).values[0]
            w_u_plus_first_time = float(self.W_U_plus.xs(i).head(1).index[0])
            w_u_min_first_state = self.W_U_min.xs(0).head(1).values[0]
            w_u_min_first_time = float(self.W_U_min.xs(i).head(1).index[0])
            w_s_plus_first_iom = w_s_plus_first_state[6]
            ...
            for row in self.W_S_min.xs(i).tail(len(self.W_S_min.xs(i)-1)).iterrows():
                w_s_min_t.append(abs(row[0]))
                w_s_min_state = row[1].values
                w_s_min_time = row[1].index
                w_s_min_iom =w_s_min_state[6]
                w_s_min_time_diff = abs(w_s_min_first_time - w_s_min_time)
                w_s_min_delta_j.append(abs(mass_theoretical)-abs(w_s_min_first_iom-w_s_min_iom)/w_s_min_time_diff)
                w_s_min_first_iom = w_s_min_iom
                w_s_min_first_time = w_s_min_time
            for row in self.W_U_plus.xs(i).tail(len(self.W_U_plus.xs(i)-1)).iterrows():
                w_u_plus_t.append(abs(row[0]))
                w_u_plus_state = row[1].values
                w_u_plus_time = row[1].index
                w_u_plus_iom = w_u_plus_state[6]
                w_u_plus_time_diff = abs(w_u_plus_first_time - w_u_plus_time)
                w_u_plus_delta_j.append(abs(mass_theoretical)-abs(w_u_plus_first_iom-w_u_plus_iom)/w_u_plus_time_diff)
                w_u_plus_first_iom = w_u_plus_iom
                w_u_plus_first_time = w_u_plus_time
            for row in self.W_U_min.xs(i).tail(len(self.W_U_min.xs(i)-1)).iterrows():
                w_u_min_t.append(abs(row[0]))
                w_u_min_state = row[1].values
                w_u_min_time = row[1].index
                w_u_min_iom = w_u_min_state[6]
                w_u_min_time_diff = abs(w_u_min_first_time - w_u_min_time)
                w_u_min_delta_j.append(abs(mass_theoretical)-abs(w_u_min_first_iom-w_u_min_iom)/w_u_min_time_diff)
                w_u_min_first_iom = w_u_min_iom
                w_u_min_first_time = w_u_min_time
        ...
        l1, = ax1.plot(
            ...
            label='$\Delta m_t^{S+} \pm 3\sigma_t^{S+} $', color=self.plottingColors['W_S_plus'], linestyle=':')
        l2, = ax1.plot(w_s_plus_df.mean(axis=1).fillna(method='ffill'), label='$\Delta m_t^{S+}$',
        ...
        l3, = ax2.plot(
            ...
            label='$\Delta m_t^{S-} \pm 3\sigma_t^{S-}$', color=self.plottingColors['W_S_min'], linestyle=':')
        l4, = ax2.plot(w_s_min_df.mean(axis=1).fillna(method='ffill'), label='$\Delta m_t^{S-}$',
        ...
        l5, = ax3.plot(
            ...
            label='$\Delta m_t^{U+} \pm 3\sigma_t^{U+}$', color=self.plottingColors['W_U_plus'], linestyle=':')
        l6, = ax3.plot(w_u_plus_df.mean(axis=1).fillna(method='ffill'), label='$\Delta m_t^{U+}$',
        ...
        l7, = ax4.plot(
            ...
            label='$\Delta m_t^{U-} \pm 3\sigma_t^{U-}$', color=self.plottingColors['W_U_min'], linestyle=':')
        l8, = ax4.plot(w_u_min_df.mean(axis=1).fillna(method='ffill'), label='$\Delta m_t^{U-}$',
        ...
        if self.thrustRestriction == 'left':
            plt.suptitle('$L_' + str(
                self.lagrangePointNr) + '$ ' + '$\{ \mathcal{W}^{S \pm}, \mathcal{W}^{U \pm} \}^{ \longleftarrow }_{' + self.thrustMagnitudeForPlotTitle + '}$' + ' - Mass verification at C = ' + str(
                np.round(self.C, 3)), size=self.suptitleSize)
        elif self.thrustRestriction == 'right':
            plt.suptitle('$L_' + str(
                self.lagrangePointNr) + '$ ' + '$\{ \mathcal{W}^{S \pm}, \mathcal{W}^{U \pm} \}^{ \longrightarrow }_{' + self.thrustMagnitudeForPlotTitle + '}$' + ' - Mass verification at C = ' + str(
                np.round(self.C, 3)), size=self.suptitleSize)
        else:
            plt.suptitle('$L_' + str(
        ...
        if self.lowDPI:
            plt.savefig('../../data/figures/manifolds/augmented/L' + str(
                ...
                self.orbitId) + '_' + self.spacecraftName + '_' + str(
                self.thrustMagnitudeForTitle) + '_' + self.thrustRestriction + '_manifold_mass_validation.png',
                        transparent=True, dpi=self.dpi)
        else:
            plt.savefig('../../data/figures/manifolds/augmented/L' + str(
                self.lagrangePointNr) + '_' + self.orbitType + '_' + str(
                self.orbitId) + '_' + self.spacecraftName + '_' + str(
                self.thrustMagnitudeForTitle) + '_' + self.thrustRestriction + '_manifold_mass_validation.pdf',
                        transparent=True)
        plt.close()
        pass

    def plot_massrate_validation(self):
        fig = plt.figure(figsize=self.figSize)
        ...
        l1, = ax1.plot(
            w_s_plus_df.mean(axis=1).fillna(method='ffill') + 3 * w_s_plus_df.std(axis=1).fillna(method='ffill'),
            label='$\dot{m}_t^{S+} \pm 3\sigma_t^{S+} $', color=self.plottingColors['W_S_plus'], linestyle=':')
        l2, = ax1.plot(w_s_plus_df.mean(axis=1).fillna(method='ffill'), label='$\dot{m}_t^{S+}$',
                       color=self.plottingColors['W_S_plus'])
        ax1.plot(w_s_plus_df.mean(axis=1).fillna(method='ffill') - 3 * w_s_plus_df.std(axis=1).fillna(method='ffill'),
                 color=self.plottingColors['W_S_plus'], linestyle=':')
        ax1.set_ylabel('$|\dot{m}^i_{t}|$ [-]')
        ax1.set_title('Mass rate on manifold ($\\forall i \in \mathcal{W}^{S+}$)', loc='right')
        ...
        l3, = ax2.plot(
            w_s_min_df.mean(axis=1).fillna(method='ffill') + 3 * w_s_min_df.std(axis=1).fillna(method='ffill'),
            label='$\dot{m}_t^{S-} \pm 3\sigma_t^{S-}$', color=self.plottingColors['W_S_min'], linestyle=':')
        l4, = ax2.plot(w_s_min_df.mean(axis=1).fillna(method='ffill'), label='$\dot{m}_t^{S-}$',
                       color=self.plottingColors['W_S_min'])
        ax2.legend(frameon=True, loc='center left', bbox_to_anchor=(1, 0.5), handles=[l1, l2, l3, l4])
        ax2.plot(w_s_min_df.mean(axis=1).fillna(method='ffill') - 3 * w_s_min_df.std(axis=1).fillna(method='ffill'),
                 color=self.plottingColors['W_S_min'], linestyle=':')
        ax2.set_ylabel('$|\dot{m}^i_{t}|$ [-]')
        ax2.set_title('Mass rate on manifold ($\\forall i \in \mathcal{W}^{S-}$)', loc='right')
        ...
        l5, = ax3.plot(
            w_u_plus_df.mean(axis=1).fillna(method='ffill') + 3 * w_u_plus_df.std(axis=1).fillna(method='ffill'),
            label='$\dot{m}_t^{U+} \pm 3\sigma_t^{U+}$', color=self.plottingColors['W_U_plus'], linestyle=':')
        l6, = ax3.plot(w_u_plus_df.mean(axis=1).fillna(method='ffill'), label='$\dot{m}_t^{U+}$',
                       color=self.plottingColors['W_U_plus'])
        ax3.plot(w_u_plus_df.mean(axis=1).fillna(method='ffill') - 3 * w_u_plus_df.std(axis=1).fillna(method='ffill'),
                 color=self.plottingColors['W_U_plus'], linestyle=':')
        ax3.set_ylabel('$|\dot{m}^i_{t}|$ [-]')
        ax3.set_title('Mass rate on manifold ($\\forall i \in \mathcal{W}^{U+}$)', loc='right')
        ax3.set_xlabel('$|t|$ [-]')
        ...
        l7, = ax4.plot(
            w_u_min_df.mean(axis=1).fillna(method='ffill') + 3 * w_u_min_df.std(axis=1).fillna(method='ffill'),
            label='$\dot{m}_t^{U-} \pm 3\sigma_t^{U-}$', color=self.plottingColors['W_U_min'], linestyle=':')
        l8, = ax4.plot(w_u_min_df.mean(axis=1).fillna(method='ffill'), label='$\dot{m}_t^{U-}$',
                       color=self.plottingColors['W_U_min'])
        ax4.legend(frameon=True, loc='center left', bbox_to_anchor=(1, 0.5), handles=[l5, l6, l7, l8])
        ax4.plot(w_u_min_df.mean(axis=1).fillna(method='ffill') - 3 * w_u_min_df.std(axis=1).fillna(method='ffill'),
                 color=self.plottingColors['W_U_min'], linestyle=':')
        ax4.set_ylabel('$|\dot{m}^i_{t}|$ [-]')
        ax4.set_title('Mass rate on manifold ($\\forall i \in \mathcal{W}^{U-}$)', loc='right')
        ax4.set_xlabel('$|t|$  [-]')
        ...
        if self.thrustRestriction == 'left':
            plt.suptitle('$L_' + str(
                self.lagrangePointNr) + '$ ' + '$\{ \mathcal{W}^{S \pm}, \mathcal{W}^{U \pm} \}^{ \longleftarrow }_{' + self.thrustMagnitudeForPlotTitle + '}$' + ' - Mass rate verification at C = ' + str(
                np.round(self.C, 3)), size=self.suptitleSize)
        elif self.thrustRestriction == 'right':
            plt.suptitle('$L_' + str(
                self.lagrangePointNr) + '$ ' + '$\{ \mathcal{W}^{S \pm}, \mathcal{W}^{U \pm} \}^{ \longrightarrow }_{' + self.thrustMagnitudeForPlotTitle + '}$' + ' - Mass rate verification at C = ' + str(
                np.round(self.C, 3)), size=self.suptitleSize)
        else:
            plt.suptitle('$L_' + str(
        ...
        if self.lowDPI:
            plt.savefig('../../data/figures/manifolds/augmented/L' + str(
                self.lagrangePointNr) + '_' + self.orbitType + '_' + str(
                self.orbitId) + '_' + self.spacecraftName + '_' + str(
                self.thrustMagnitudeForTitle) + '_' + self.thrustRestriction + '_manifold_massrate_validation.png',
                        transparent=True, dpi=self.dpi)
        else:
            plt.savefig('../../data/figures/manifolds/augmented/L' + str(
                self.lagrangePointNr) + '_' + self.orbitType + '_' + str(
                self.orbitId) + '_' + self.spacecraftName + '_' + str(
                self.thrustMagnitudeForTitle) + '_' + self.thrustRestriction + '_manifold_massrate_validation.pdf',
                        transparent=True)
        plt.close()
        pass
