...
class RichardsonApproximation:
    ...
    def compute_coefficients(self, type, lagrange_point_nr):
        ...
        if lagrange_point_nr == 1:
            gammaL = nsolve(gammaL ** 5 - (3 - mu) * gammaL ** 4 + (
            3 - 2 * mu) * gammaL ** 3 - mu * gammaL ** 2 + 2 * mu * gammaL - mu, gammaL, 1)
        ...
        s2 = 1 / (2 * l * (l * (1 + k ** 2) - 2 * k)) * (
            3 / 2 * c3 * (2 * a22 * (k ** 2 - 2) + a24 * (k ** 2 + 2) + 2 * k * b22 + 5 * d21) + 3 / 8 * c4 * (
            12 - k ** 2))
        ...
        df = pd.DataFrame({column_name: [gammaL, l, k, delta, c2, c3, c4, s1, s2, l1, l2, a1, a2, d1, d2, a21, a22, a23,
                                         a24, a31, a32, b21, b22, b31, b32, d21, d31, d32]},
                          index=['gammaL', 'l', 'k', 'delta', 'c2', 'c3', 'c4', 's1', 's2', 'l1', 'l2', 'a1', 'a2',
