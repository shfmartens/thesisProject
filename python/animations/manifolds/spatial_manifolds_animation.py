...
class SpatialManifoldsAnimation:
    ...
    def update_lines(self, i):
        ...
        #         pass
        #     if self.numberOfOrbitsPerManifold <= j < self.numberOfOrbitsPerManifold * 2:
        #         x = self.W_U_plus[0].xs(j - self.numberOfOrbitsPerManifold * 1)['x'].tolist()
        #         y = self.W_U_plus[0].xs(j - self.numberOfOrbitsPerManifold * 1)['y'].tolist()
        #         z = self.W_U_plus[0].xs(j - self.numberOfOrbitsPerManifold * 1)['z'].tolist()
        #         pass
        #     if self.numberOfOrbitsPerManifold * 2 <= j < self.numberOfOrbitsPerManifold * 3:
        #         x = self.W_U_min[0].xs(j - self.numberOfOrbitsPerManifold * 2)['x'].tolist()
        #         y = self.W_U_min[0].xs(j - self.numberOfOrbitsPerManifold * 2)['y'].tolist()
        #         z = self.W_U_min[0].xs(j - self.numberOfOrbitsPerManifold * 2)['z'].tolist()
        #         pass
        #     if self.numberOfOrbitsPerManifold * 3 <= j < self.numberOfOrbitsPerManifold * 4:
        #         x = self.W_S_min[0].xs(j - self.numberOfOrbitsPerManifold * 3)['x'].tolist()
        #         y = self.W_S_min[0].xs(j - self.numberOfOrbitsPerManifold * 3)['y'].tolist()
        #         z = self.W_S_min[0].xs(j - self.numberOfOrbitsPerManifold * 3)['z'].tolist()
        #         pass
        #     if self.numberOfOrbitsPerManifold * 4 <= j < self.numberOfOrbitsPerManifold * 5:
        ...
        #         pass
        #     if self.numberOfOrbitsPerManifold * 5 <= j < self.numberOfOrbitsPerManifold * 6:
        #         x = self.W_U_plus[1].xs(j - self.numberOfOrbitsPerManifold * 5)['x'].tolist()
        #         y = self.W_U_plus[1].xs(j - self.numberOfOrbitsPerManifold * 5)['y'].tolist()
        #         z = self.W_U_plus[1].xs(j - self.numberOfOrbitsPerManifold * 5)['z'].tolist()
        #     if self.numberOfOrbitsPerManifold * 6 <= j < self.numberOfOrbitsPerManifold * 7:
        #         x = self.W_U_min[1].xs(j - self.numberOfOrbitsPerManifold * 6)['x'].tolist()
        #         y = self.W_U_min[1].xs(j - self.numberOfOrbitsPerManifold * 6)['y'].tolist()
        #         z = self.W_U_min[1].xs(j - self.numberOfOrbitsPerManifold * 6)['z'].tolist()
        #         pass
        #     if self.numberOfOrbitsPerManifold * 7 <= j < self.numberOfOrbitsPerManifold * 8:
        #         x = self.W_S_min[1].xs(j - self.numberOfOrbitsPerManifold * 7)['x'].tolist()
        #         y = self.W_S_min[1].xs(j - self.numberOfOrbitsPerManifold * 7)['y'].tolist()
        #         z = self.W_S_min[1].xs(j - self.numberOfOrbitsPerManifold * 7)['z'].tolist()
        #         pass
    ...
    def animate(self):
        ...
        self.lines = [self.ax.plot([], [], color=color_palette_red[idx], alpha=self.orbitAlpha)[0] for idx in range(self.numberOfOrbitsPerManifold)]
        self.lines.extend([self.ax.plot([], [], color=color_palette_green[idx], alpha=self.orbitAlpha)[0] for idx in range(self.numberOfOrbitsPerManifold)])
        self.lines.extend([self.ax.plot([], [], color=color_palette_red[idx], alpha=self.orbitAlpha)[0] for idx in range(self.numberOfOrbitsPerManifold)])
        self.lines.extend([self.ax.plot([], [], color=color_palette_green[idx], alpha=self.orbitAlpha)[0] for idx in range(self.numberOfOrbitsPerManifold)])
        self.lines.extend([self.ax.plot([], [], color=color_palette_red[idx], alpha=self.orbitAlpha)[0] for idx in range(self.numberOfOrbitsPerManifold)])
        self.lines.extend([self.ax.plot([], [], color=color_palette_green[idx], alpha=self.orbitAlpha)[0] for idx in range(self.numberOfOrbitsPerManifold)])
        self.lines.extend([self.ax.plot([], [], color=color_palette_green[idx], alpha=self.orbitAlpha)[0] for idx in range(self.numberOfOrbitsPerManifold)])
        ...
        self.file_name = '../../../data/animations/manifolds/spatial_manifolds_' + self.orbitType + '_' + str(self.cLevel) + '.mp4'
...
if __name__ == '__main__':
    ...
    for orbit_type in orbit_types:
        for c_level in c_levels:
            spatial_manifolds_animation = SpatialManifoldsAnimation(orbit_type, c_level, orbit_ids)
            spatial_manifolds_animation.animate()
            # list_of_objects.append(SpatialManifoldsAnimation(orbit_type, c_level, orbit_ids))  # Instantiate the classes
    #
    ...
    # pool1 = mp.Pool(min(maximum_number_of_processes, len(list_of_objects)))  # Start a pool of workers
    # pool1.imap_unordered(worker, (obj for obj in list_of_objects))  # Execute the tasks in parallel, regardless of order
    # pool1.close()  # No more tasks can be submitted to the pool
    # pool1.join()  # Wait for processes in pool to exit
