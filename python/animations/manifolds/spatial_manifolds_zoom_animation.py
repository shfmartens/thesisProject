...
class SpatialManifoldsZoomAnimation:
    ...
    def update_lines(self, i):
        ...
        for j, line in enumerate(self.lines):
            ...
            for k in range(len(x)):
                if x[k] < self.xLim[0] or x[k] > self.xLim[1] or \
                                y[k] < self.yLim[0] or y[k] > self.yLim[1] or \
                                z[k] < self.zLim[0] or z[k] > self.zLim[1]:
                    x[k] = pylab.NaN
                    y[k] = pylab.NaN
                    z[k] = pylab.NaN
    ...
    def animate(self):
        ...
        self.file_name = '../../../data/animations/manifolds/spatial_manifolds_zoom_' + self.orbitType + '_' + str(self.cLevel) + '.mp4'
...
if __name__ == '__main__':
    ...
    for orbit_type in orbit_types:
        for c_level in c_levels:
            spatial_manifolds_zoom_animation = SpatialManifoldsZoomAnimation(orbit_type, c_level, orbit_ids)
            spatial_manifolds_zoom_animation.animate()
            # list_of_objects.append(SpatialManifoldsZoomAnimation(orbit_type, c_level, orbit_ids))  # Instantiate the classes
    #
    ...
    # pool4 = mp.Pool(min(maximum_number_of_processes, len(list_of_objects)))  # Start a pool of workers
    # pool4.imap_unordered(worker, (obj for obj in list_of_objects))  # Execute the tasks in parallel, regardless of order
    # pool4.close()  # No more tasks can be submitted to the pool
    # pool4.join()  # Wait for processes in pool to exit
