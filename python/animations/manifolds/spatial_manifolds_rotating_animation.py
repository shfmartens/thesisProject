...
class SpatialManifoldsRotatingAnimation:
    ...
    def animate(self):
        ...
        title = self.orbitTypeForTitle + ' $\{ \mathcal{W}^{S \pm}, \mathcal{W}^{U \pm} \}$ - Orthografic projection (C = ' + str(c_level) + ')'
        ...
        self.file_name = '../../../data/animations/manifolds/spatial_manifolds_rotating_' + self.orbitType + '_' + str(self.cLevel) + '.mp4'
...
if __name__ == '__main__':
    ...
    for orbit_type in orbit_types:
        for c_level in c_levels:
            ...
            del spatial_manifolds_rotating_animation
            # list_of_objects.append(SpatialManifoldsRotatingAnimation(orbit_type, c_level, orbit_ids))  # Instantiate the classes
    #
    ...
    # pool2 = mp.Pool(min(maximum_number_of_processes, len(list_of_objects)))  # Start a pool of workers
    # pool2.imap_unordered(worker, (obj for obj in list_of_objects))  # Execute the tasks in parallel, regardless of order
    # pool2.close()  # No more tasks can be submitted to the pool
    # pool2.join()  # Wait for processes in pool to exit
