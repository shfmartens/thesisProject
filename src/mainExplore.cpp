    ...
    #pragma omp parallel num_threads(1)
    {
        ...
        for (unsigned int i=1; i<=1; i++) {
            if (i ==1)
            {
                ...
                int numberOfFamilyMembers = 25;
                bool startContinuationFromTextFile = false;
                createLowThrustInitialConditions(5, ySign, orbitType, continuationIndex, accelerationMagnitude, accelerationAngle, accelerationAngle2, initialMass, familyHamiltonian, startContinuationFromTextFile, numberOfFamilyMembers );
