                                                           ...
                                                           double amplitude, double accelerationMagnitude, double thrustAngle1, double thrustAngle2, double initialMass, int numberOfPatchPoints, double n= 1.0 )
{
    ...
    Eigen::VectorXd stateVectorInclTimeAtPatchPoint(11);
    Eigen::VectorXd initialStateVectorInclPeriod(11*numberOfPatchPoints);
    stateVectorInclTimeAtPatchPoint.setZero();
    initialStateVectorInclPeriod.setZero();
    ...
    // A loop should start here, tau should vary between zero and 2 pi
        // orbital period should be computed first,
        // as a function of numberOfpatchpoints and current patch points, tau should be tweaked!
        // then the computation of velocities and positions can be computed
        //
    ...
    for (int i = 0; i < numberOfPatchPoints; i++){
        ...
        double currentTime = periodVariable * orbitalPeriod / (periodVariable2 - 1.0);
        double tau1;
        if ( i == (numberOfPatchPoints - 1) )
        {
            //tau1   = 0.0;
            //tau1   = 2.0 * tudat::mathematical_constants::PI * (  (currentTime) / orbitalPeriod );
            tau1 = 0.0;
        } else{
        ...
        if (librationPointNr == 1){
            stateVectorInclTimeAtPatchPoint(0) = (x - 1.0) * gammaL + 1.0 - massParameter;
        } if (librationPointNr == 2){
            stateVectorInclTimeAtPatchPoint(0) = (x + 1.0) * gammaL + 1.0 - massParameter;
        }

\
        stateVectorInclTimeAtPatchPoint(1)  = y * gammaL;
        stateVectorInclTimeAtPatchPoint(2)  = z * gammaL;
        stateVectorInclTimeAtPatchPoint(3)  = xdot * gammaL;
        stateVectorInclTimeAtPatchPoint(4)  = ydot * gammaL;
        stateVectorInclTimeAtPatchPoint(5)  = zdot * gammaL;
        stateVectorInclTimeAtPatchPoint(6)  = accelerationMagnitude;
        stateVectorInclTimeAtPatchPoint(7)  = thrustAngle1;
        stateVectorInclTimeAtPatchPoint(8)  = thrustAngle2;
        stateVectorInclTimeAtPatchPoint(9)  = initialMass;

        if ( i == (numberOfPatchPoints - 1) )
        {
            stateVectorInclTimeAtPatchPoint(10) = orbitalPeriod;


        } else {

            stateVectorInclTimeAtPatchPoint(10) = currentTime;

        }

         initialStateVectorInclPeriod.segment( i*11 ,11) = stateVectorInclTimeAtPatchPoint;
