...
//std::cout << "\n MASS RATE CURRENTLY SET TO ZERO !!!!!!!!!!!!!" << std::endl;
    ...
    for (int i = 1; i < (numberOfPatchPoints-1); i++)
    {
        ...
        // Extract the necessary information from propagatedStates and initialGuess
...
//                Eigen::MatrixXd partialVelocityPlusPositionP = (B_FP.inverse());
//                Eigen::VectorXd partialVelocityPlusTimeP = -1.0*(B_FP.inverse())*velocityF;
//                Eigen::MatrixXd partialVelocityPlusPositionF = -1.0 * (B_FP.inverse())*A_FP;
//                Eigen::VectorXd partialVelocityPlusTimeF = (B_FP.inverse())*(A_FP*velocityPPlus + E_FP*massRatePPlus)+accelerationsPPlus;
        ...
        if (i == numberOfPatchPoints-2 )
        {
            ...
            periodicityJacobianRow2.block(0,(4*(i+1))+3,3,1) =  B_PF.inverse() * A_PF * velocityF + accelerationsF;

            //SHOULDN'T IT BE?t << "TEST: "<< B_PF.inverse() * A_PF * velocityPMinus + accelerationsF << std::endl;
